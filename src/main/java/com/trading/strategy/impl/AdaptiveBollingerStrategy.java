...
public class AdaptiveBollingerStrategy extends AbstractTradingStrategy {
    ...
    private enum MarketState { TRENDING, RANGING, VOLATILE }
    ...
    public TradingSignal generateSignal(MarketData marketData, List<TechnicalIndicators> indicatorHistory, List<Position> positions) {
        ...
        MarketState marketState = identifyMarketState(indicators);
    ...
    private MarketState identifyMarketState(TechnicalIndicators indicators) {
        BigDecimal adx = indicators.getAdx();
        BigDecimal atr = indicators.getAtr();
        BigDecimal close = indicators.getMiddleBand(); // Use middle band as a proxy for close
    ...
    private TradingSignal handleRangingMarket(MarketData marketData, TechnicalIndicators indicators, List<Position> positions) {
        ...
        if (bb == null) return createNoActionSignal("缺少盘整市参数的布林带指标: " + paramsKey);

        // 在盘整市，使用均值回归逻辑
        BigDecimal price = marketData.getClose();
        boolean hasPosition = positions.stream().anyMatch(p -> p.getSymbol().equals(marketData.getSymbol()) && p.getQuantity() > 0);
    ...
    private TradingSignal handleTrendingMarket(MarketData marketData, TechnicalIndicators indicators, List<Position> positions) {
        String paramsKey = config.getAdaptive().getTrendingParamsKey();
        TechnicalIndicators.BollingerBandSet bb = indicators.getBollingerBands().get(paramsKey);
        if (bb == null) return createNoActionSignal("缺少趋势市参数的布林带指标: " + paramsKey);
    ...
    private TradingSignal handleVolatileMarket(MarketData marketData, TechnicalIndicators indicators, List<Position> positions) {
    ...
    // We can leave calculatePositionSize and applyRiskManagement with default implementations from AbstractTradingStrategy for now
    @Override
