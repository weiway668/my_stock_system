...
class BacktestEngineTest {
    ...
    void testLimitOrderExecution() throws Exception {
        ...
        // 2. 准备模拟市场数据
        MarketData bar1 = MarketData.builder().symbol("HK.00700").timestamp(request.getStartTime()).low(new BigDecimal("291")).high(new BigDecimal("295")).close(new BigDecimal("294")).build();
        MarketData bar2 = MarketData.builder().symbol("HK.00700").timestamp(request.getEndTime()).low(new BigDecimal("288")).high(new BigDecimal("292")).close(new BigDecimal("290")).build();
        when(marketDataService.getOhlcvData(anyString(), anyString(), any(LocalDateTime.class), any(LocalDateTime.class), anyInt()))
        ...
        // 4. 模拟费用计算器
        when(commissionCalculator.calculateCommission(any(BigDecimal.class), anyLong(), anyString(), anyBoolean()))
                .thenReturn(HKStockCommissionCalculator.CommissionBreakdown.builder().totalCost(BigDecimal.ZERO).build());
